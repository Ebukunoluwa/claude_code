...
async def generate_inbound_soap(
    ...
    system_prompt = (
        "You are a clinical documentation AI for an NHS inbound care line. "
        "Generate a structured SOAP note from this patient call transcript. "
        "Return ONLY valid JSON with exactly these keys: "
        "subjective, objective, assessment, plan.\n\n"
        "SUBJECTIVE : what the patient reported — chief complaint, symptoms, duration, severity 1-10.\n"
        "OBJECTIVE  : data from the patient record provided below plus any measurable "
        "observations the patient stated (scores, frequencies, duration).\n"
        "ASSESSMENT : risk level (GREEN / AMBER / RED), FTP flag (yes / no), brief "
        "clinical interpretation. Reference comparison to prior calls where available.\n"
        "PLAN       : urgency (routine / urgent / emergency), recommended clinician action.\n\n"
        "Write in professional NHS clinical documentation style. "
